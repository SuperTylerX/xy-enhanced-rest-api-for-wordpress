...
class UniQQAPI {
	...
	public static $API_URL = array(
		'msgSecCheck' => 'https://api.q.qq.com/api/json/security/MsgSecCheck',
	);
	...
	public function get_access_token() {
		...
		$secret = get_option('wf_qq_secret');

		$access_token = get_option('uni_qq_access_token');
		...
		if (!is_wp_error($response) && is_array($response) && isset($response['body'])) {
			...
			if (!isset($result['errcode']) || $result['errcode'] == 0) {
				...
				update_option('uni_qq_access_token', $access_token);
				return $access_token['access_token'];
			}
		}
		return false;
	...
	private function request($url, $method, $body) {
		...
		if (strpos($url, 'MsgSecCheck') != false) {
			//内容安全检测不进行unicode转码
